...
class DatabaseManager:
    ...
    def define_models(self):
        ...
        class TableServiceViolation(self.db.Model):
            ...
            waiting_time = self.db.Column(self.db.Float, nullable=False)  # Waiting time in seconds
            snapshot_filename = self.db.Column(self.db.String(255), nullable=True)
            snapshot_path = self.db.Column(self.db.String(500), nullable=True)
            alert_data = self.db.Column(self.db.Text)  # JSON data with detailed violation info
            file_size = self.db.Column(self.db.Integer)  # File size in bytes (if snapshot exists)
        ...
        try:
            ...
            for ch in channel_stats:
                channels_data.append({
                    ...
                    'last_detection': ch.last_detection.isoformat() if ch.last_detection else None
                })
            ...
            for d in daily_stats:
                try:
                    ...
                    logger.error(f"Error formatting daily stat date: {e}, type: {type(d.snapshot_date)}, value: {d.snapshot_date}")
                    daily_data.append({
                        'date': None,
                        'snapshot_count': d.count,
                        'detections': int(d.detections or 0)
            ...
            return [
                {
                    ...
                    'file_size': snap.file_size,
                    'created_at': snap.created_at.isoformat(),
                    'snapshot_url': f'/static/fall_snapshots/{snap.snapshot_filename}'
                } for snap in snapshots
            ]
    ...
    def add_table_service_violation(self, channel_id, table_id, waiting_time, snapshot_path=None, timestamp=None, alert_data=None):
        ...
        try:
            ...
            logger.info(f"Table service violation saved: Table {table_id} in channel {channel_id}, waiting time: {waiting_time:.1f}s")
            return violation.id
    ...
    def get_table_service_violations(self, channel_id=None, table_id=None, limit=50):
